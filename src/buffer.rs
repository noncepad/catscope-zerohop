...
mod tests {
    ...
    fn test_single_producer_consumer() {
        ...
        let tx = CatscopeTransaction::default();
        producer.push(tx);
    ...
    fn test_multiple_producers() {
        let ring = TransactionRing::new();
        let producer1 = ring.producer();
        let producer2 = ring.producer();

        let tx1 = CatscopeTransaction::default();
        let tx2 = CatscopeTransaction::default();

        let seq1 = producer1.push(tx1);
        let seq2 = producer2.push(tx2);
    ...
    fn test_broadcast_to_multiple_consumers() {
        ...
        // Both consumers should see the same transaction
        let result1 = consumer1.try_pop();
        let result2 = consumer2.try_pop();
    ...
    #[test]
    fn test_consumer_lag_detection() {
        let ring = TransactionRing::new();
        let producer = ring.producer();
        ...
        for _ in 0..(RING_SIZE + 100) {
            producer.push(CatscopeTransaction::default());
        }

        assert!(consumer.is_lapped());
        assert!(consumer.lag() > RING_SIZE as u64);
    }
}
